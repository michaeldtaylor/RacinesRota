...
function Set-RotaColumnWidths {
    <#
    .SYNOPSIS
        Size a sheet's columns, falling back to measuring the text ourselves.
    .DESCRIPTION
        EPPlus sizes columns by asking the platform to measure a rendered string, which on
        macOS and bare Linux means libgdiplus. Where that is missing the call does not degrade
        -- it throws, and because the workbook is built in one pass the whole export is lost
        with it. Losing the rota because a column is the wrong width is a poor trade.

        So: try the real thing, and if the platform cannot do it, measure the longest string
        in each column and set a width from that. The result is slightly less pretty and
        entirely readable, and it needs nothing installed.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]$Worksheet,
        # Rows to leave out of the measurement. A one-line footnote sitting in column A is
        # far longer than anything in the column proper, and sizing to it leaves a name
        # column wide enough for a paragraph.
        [int[]]$IgnoreRows = @()
    )
    ...
    try {
        ...
        Write-Verbose "AutoFitColumns unavailable ($($_.Exception.Message)); measuring instead."
    }
    ...
    for ($c = $dim.Start.Column; $c -le $dim.End.Column; $c++) {
        ...
        for ($r = $dim.Start.Row; $r -le $dim.End.Row; $r++) {
            if ($IgnoreRows -contains $r) { continue }
            $text = "$($Worksheet.Cells[$r, $c].Value)"
            if ($text.Length -gt $longest) { $longest = $text.Length }
        }
        # A little padding, and a ceiling so one long note does not push a column off-screen.
        $Worksheet.Column($c).Width = [math]::Min([math]::Max($longest + 2, 8), 60)
    }
}
...
function Write-RotaGridSheet {
    ...
    # The footnote overflows across the empty cells beside it, so it must not set column A's width.
    Set-RotaColumnWidths -Worksheet $ws -IgnoreRows $row
}
